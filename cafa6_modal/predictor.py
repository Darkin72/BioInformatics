...
from cafa6_modal.models import BiLSTMAttention, EmbeddingMLP, ProtCNN
...
class CAFA6Predictor:
    ...
    def predict(
        ...
        predictions = self._prediction_rows(
            normalized, ensemble_probs, "ensemble", top_k, threshold
        )
        result = {
    ...
    def _load_branch_states(self):
        ...
        full_ckpt = self.artifact_dir / "cafa6_high_performance_models.pt"
    ...
    def _load_runtime_models(self):
        if "esm_mlp" in self.model_states:
            ...
            # Infer embedding dimension from the checkpoint to avoid hard-coding ESM sizes.
